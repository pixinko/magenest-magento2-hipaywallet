...
class HipayWalletPayment extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            $this->hipayLogger->debug('Product name : ' .
                $item->getProduct()->getName());

            $stockItem = $this->stockRegistry->getStockItem($product->getId());
            $this->hipayLogger->debug('Product quantity before : ' . $stockItem->getQty());
            
            $stockItem->setQty($stockItem->getQty() + $item->getQtyOrdered());
            $stockItem->save();
            $this->hipayLogger->debug('Product quantity after : ' . $stockItem->getQty());
        }
    ...
    public function authorize(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        // Decrease stock properly. Cannot be below orderSender->send()
        foreach ($order->getAllItems() as $item) {
            ...
            $this->hipayLogger->debug('Product quantity ordered : ' . $item->getQtyOrdered());
